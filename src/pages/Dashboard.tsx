...
import { Clock, CheckCircle, XCircle, FileText, Timer } from "lucide-react";
...
const DISCORD_ICON = (
  <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" fillRule="evenodd" clipRule="evenodd">
...
// MUST be defined before Dashboard component (const is not hoisted)
function getCooldownInfo(updatedAt: string) {
  const COOLDOWN_MS = 12 * 60 * 60 * 1000;
  const rejectedAt = new Date(updatedAt).getTime();
  const remaining = COOLDOWN_MS - (Date.now() - rejectedAt);
...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    rawSelect<Application[]>("applications", {
      user_id: `eq.${user.id}`,
        ...
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          {error ? (
            <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-8 text-center">
              ...
              <h2 className="font-heading text-xl font-semibold text-red-300 mb-2">Error Loading Applications</h2>
            ...
            <div className="space-y-4"><AppSkeleton /><AppSkeleton /></div>
            ...
            <div className="bg-card border border-border rounded-xl p-12 text-center">
              ...
              <Link to="/apply" className="gradient-red text-primary-foreground px-8 py-3 rounded-md font-heading uppercase tracking-wider hover:box-glow-red transition-all inline-block">
                Apply Now
              </Link>
            </div>
          ) : (
                ...
                return (
                  <div key={app.id} className="bg-card border border-border rounded-xl p-6">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="min-w-0">
                          <div className="flex items-center gap-2 flex-wrap">
                            <h3 className="font-heading text-lg font-semibold truncate">{app.char_name || "Unknown"}</h3>
                    ...
                    {app.status === "rejected" && (
                      <div className="mt-4">
                        {(() => {
                          const cooldown = getCooldownInfo(app.updated_at);
                          if (!cooldown) {
                            return (
                              <div className="flex items-center gap-2 text-green-400 text-sm">
                                <CheckCircle size={16} />
                                <span className="font-heading uppercase tracking-wide">Cooldown expired — you can re-apply now</span>
                              </div>
                            );
                          }
                          return (
                            <div className="flex items-center gap-3 p-3 bg-red-500/10 border border-red-500/30 rounded-lg">
                              ...
                              <div>
                                <p className="text-xs text-red-400 uppercase tracking-wide font-heading">Under Cooldown</p>
                                <p className="text-sm text-red-300">
                                  You can re-apply in <span className="font-bold">{cooldown.hours}h {cooldown.minutes}m</span>
                                </p>
                              </div>
                            </div>
                          );
